@extends('layout')
@section('title', 'Pre-Test: ' . $module->title . ' — CerdasFin')
...
<div class="container-cf py-12 max-w-3xl">
    ...
    <form method="POST" action="{{ route('modules.pre-test.submit', $module) }}" x-data="quizForm({{ $quiz->questions->count() }})">
        @csrf
        <div class="space-y-6">
            ...
            <div class="card p-6 animate-fade-in-up" style="animation-delay: {{ $i * 0.1 }}s">
                ...
                <div class="space-y-2 pl-11">
                    ...
                    <label class="quiz-option" :class="selected[{{ $question->id }}] === {{ $j }} ? 'selected' : ''">
                        ...
                        <div class="w-5 h-5 rounded-full border-2 border-current flex-shrink-0 flex items-center justify-center" :class="selected[{{ $question->id }}] === {{ $j }} ? 'border-deep-fern-green' : 'border-gray-300'">
        ...
        <div class="mt-8 flex items-center justify-between">
            <a href="{{ route('modules.show', $module) }}" class="btn-ghost">← Kembali</a>
